...
const ratings = watchList.map((elem, index) => ({title: elem["Title"],rating: elem["imdbRating"]}));
...
const filteredList = watchList.map(elem => ({title: elem["Title"], rating: elem["imdbRating"]})).filter(elem => elem.rating >= 8);
// Only change code above this line
...
function getRating(watchList) {
    ...
    const filterList = watchList.map(elem => ({director: elem["Director"], rating: +elem["imdbRating"]})).filter(elem => elem.director === "Christopher Nolan");


    let sumRaiting = filterList.reduce((sum, elem) => sum + elem.rating, 0);

    averageRating = sumRaiting / filterList.length;
...
const squareList = arr => {
    // Only change code below this line
    const newArr = arr.filter(elem => elem > 0 && Number.isInteger(elem)).map(elem => elem * elem);
...
function urlSlug(title) {
    const arr = title.trim().split(/\s+/gi);
